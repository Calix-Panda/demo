...
<body>
	...
	<h1 class="jumbotron-heading">公告一覽</h1>
		...
		<s:form action="removeByID">
			<s:submit type="button" value="刪除已選擇的" align="left"
				onClick="return confirm('Do you want to delete these contacts?');" />
			<display:table id="row" class="table dataTable" export="true"
				name="informationList" size="auto" pagesize="5" cellpadding="5px;"
				cellspacing="5px;" requestURI="">
				<display:setProperty name="paging.banner.placement" value="bottom" />
				<display:column title="選擇">
					<s:checkbox id="check" name="Checkbox" fieldValue="%{#attr.row.id}"
						theme="simple" value="#{attr.row.check}" />
				</display:column>
				...
				<display:column title="">
					<s:a href="%{delUrl}" onclick="return confirm('您確定要刪除嗎?')">刪除</s:a>
				</display:column>
			</display:table>
		</s:form>
		<s:a action="forwordInfomation">點我新增</s:a>
	</div>
</body>
</html>